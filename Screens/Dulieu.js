...
const Dulieu = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const snapshot = await getDocs(collection(db, 'dulieuquantrac'));
        const list = snapshot.docs.map(doc => {
          ...
          return {
            ...
            pdf_link: d.pdf_link,
            date: d.created_at?.toDate?.().toLocaleDateString('vi-VN') ?? '', // Chuyển Timestamp thành chuỗi
          };
        });
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.card} onPress={() => openUrl(item.pdf_link)}>
      {item.thumbnail && <Image source={{ uri: item.thumbnail }} style={styles.image} />}
      <Text style={styles.title}>{item.title}</Text>
      <Text style={styles.meta}>📅 {item.date}</Text>
      <View style={styles.button}>
        <Text style={styles.buttonText}>Xem file PDF</Text>
      </View>
    </TouchableOpacity>
  );
